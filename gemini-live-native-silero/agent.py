"""
Voice agent using Google Gemini Live API with Silero VAD-based turn detection.
...
- Uses Silero VAD (ONNX) for client-side voice activity detection
...
Configuration (via environment variables):
    ...
    VAD_CONFIDENCE: Silero VAD confidence threshold 0.0-1.0 (default: 0.5)
"""
...
class GeminiVoiceBot:
    ...
    def __init__(
        ...
        self._silero = SileroVAD(model_path)
...
You can use the caller's phone number for SMS or callbacks without asking."""
    ...
    def _process_vad_frame(self, pcm_8k: bytes) -> None:
        """Buffer PCM and run Silero VAD when enough data is accumulated.

        Silero needs 256 samples (512 bytes PCM16) at 8kHz per frame.
        Plivo sends 160 bytes μ-law = 320 bytes PCM16 (160 samples).
        So we buffer ~2 Plivo frames to get one Silero frame.

        Args:
            pcm_8k: PCM16 audio at 8kHz (variable length from Plivo)
        """
        self._vad_buffer.extend(pcm_8k)

        while len(self._vad_buffer) >= VAD_FRAME_BYTES:
            frame_bytes = bytes(self._vad_buffer[:VAD_FRAME_BYTES])
            self._vad_buffer = self._vad_buffer[VAD_FRAME_BYTES:]

            try:
                confidence = self._silero.process_frame(frame_bytes)
            except Exception:
                continue
            ...
            if is_speech:
                ...
                if (
                    self._vad_speech_count >= VAD_SPEECH_FRAMES_THRESHOLD
                    and not self._user_speaking
                ):
                    self._user_speaking = True
                    logger.debug(f"VAD: user speech started (confidence={confidence:.2f})")
                ...
                if (
                    self._vad_silence_count >= VAD_SILENCE_FRAMES_THRESHOLD
