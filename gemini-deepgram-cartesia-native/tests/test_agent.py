...
class TestCartesiaTTS:
    ...
    @pytest.mark.asyncio
    async def test_initialization(self):
        """Test CartesiaTTS initialization."""
        tts = CartesiaTTS()
    ...
    async def test_synthesize_success(self):
        ...
        assert "api.cartesia.ai" in call_args[0][0]
...
class TestVoiceAgent:
    ...
    async def test_initialization(self):
        ...
        with patch("agent.genai.Client"):
            ...
            assert agent.call_id == "test-123"
            assert agent.from_number == "+15551234567"
            assert agent.to_number == "+15559876543"
    ...
    async def test_handle_transcript_generates_response(self):
        ...
        with patch("agent.genai.Client") as mock_genai:
            ...
            agent = VoiceAgent(websocket=mock_ws, call_id="test-123")

            # Mock TTS to return audio
            agent._tts.synthesize = AsyncMock(return_value=b"\x00" * 4800)

            await agent._handle_transcript("Hello, I need help.")

            # Should have called LLM and TTS
            mock_client.aio.models.generate_content.assert_called_once()
            agent._tts.synthesize.assert_called_once()
    ...
    async def test_tts_audio_format_conversion(self):
        ...
        # Simulate Cartesia output (24kHz PCM)
        import numpy as np
        from agent import resample_audio, CARTESIA_SAMPLE_RATE, PLIVO_SAMPLE_RATE

        # 100ms of audio at 24kHz
        samples_24k = int(CARTESIA_SAMPLE_RATE * 0.1)
...
class TestIntegration:
    ...
    async def test_full_conversation_flow(self):
        ...
        with patch("agent.genai.Client") as mock_genai:
            ...
            agent = VoiceAgent(websocket=mock_ws, call_id="integration-test")

            # Mock the services
            agent._stt.connect = AsyncMock()
            agent._stt.close = AsyncMock()
            agent._stt.send_audio = AsyncMock()

            agent._tts.connect = AsyncMock()
            agent._tts.close = AsyncMock()
            # Return 24kHz PCM audio (100ms)
            agent._tts.synthesize = AsyncMock(return_value=b"\x00" * 4800)

            # Simulate a transcript being received
            await agent._handle_transcript("Hi, I have a question about billing.")

            # Verify the LLM was called
            assert mock_client.aio.models.generate_content.called

            # Verify TTS was called with the response
            agent._tts.synthesize.assert_called()

            # Verify audio was queued for sending
            assert not agent._audio_queue.empty()
    ...
    async def test_concurrent_task_handling(self):
        ...
        with patch("agent.genai.Client"):
            agent = VoiceAgent(websocket=mock_ws, call_id="concurrent-test")

            # Verify the agent can be initialized with proper async primitives
            assert isinstance(agent._audio_queue, asyncio.Queue)
            assert isinstance(agent._processing_lock, asyncio.Lock)
