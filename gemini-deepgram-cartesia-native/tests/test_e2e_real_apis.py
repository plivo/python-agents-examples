...
class TestGeminiLLMReal:
    ...
    async def test_gemini_customer_service_prompt(self):
        ...
        from agent import GeminiLLM, DEFAULT_SYSTEM_PROMPT
...
class TestCartesiaTTSReal:
    """End-to-end tests for Cartesia TTS with real API."""

    @pytest.mark.asyncio
    async def test_cartesia_synthesize_short_text(self):
        """Test synthesizing short text."""
        from agent import CartesiaTTS
        ...
        try:
            ...
            print(f"\n[Cartesia]: Synthesized {len(audio)} bytes of audio")
            print(f"[Cartesia]: Duration ~{len(audio) / 48000:.2f} seconds")

        finally:
            await tts.close()

    @pytest.mark.asyncio
    async def test_cartesia_synthesize_longer_text(self):
        ...
        try:
            ...
            print(f"\n[Cartesia]: Synthesized {len(audio)} bytes of audio")
            print(f"[Cartesia]: Duration ~{duration_seconds:.2f} seconds")

        finally:
            await tts.close()

    @pytest.mark.asyncio
    async def test_cartesia_audio_format(self):
        """Test that Cartesia returns valid PCM audio."""
        from agent import CartesiaTTS
        ...
        try:
            ...
            print(f"\n[Cartesia]: Audio has {len(samples)} samples")
            print(f"[Cartesia]: Sample range: [{samples.min()}, {samples.max()}]")
            print(f"[Cartesia]: Standard deviation: {np.std(samples):.2f}")
...
class TestFullPipelineReal:
    ...
    async def test_llm_to_tts_pipeline(self):
        ...
        from agent import GeminiLLM, CartesiaTTS, resample_audio, pcm_to_ulaw
        from agent import CARTESIA_SAMPLE_RATE, PLIVO_SAMPLE_RATE
    ...
    async def test_audio_conversion_pipeline(self):
        ...
        try:
            ...
            pcm_8k = resample_audio(original_audio, CARTESIA_SAMPLE_RATE, PLIVO_SAMPLE_RATE)
    ...
    async def test_conversation_flow(self):
        ...
        from agent import GeminiLLM, CartesiaTTS, DEFAULT_SYSTEM_PROMPT
