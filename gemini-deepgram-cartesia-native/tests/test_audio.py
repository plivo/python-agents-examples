...
class TestAudioPipeline:
    ...
    def test_cartesia_to_plivo_format(self):
        """Test Cartesia (PCM 24kHz) to Plivo (u-law 8kHz) conversion."""
        # Simulate Cartesia TTS output (24kHz PCM)
        duration_ms = 20
        samples_24k = int(CARTESIA_SAMPLE_RATE * duration_ms / 1000)
        cartesia_audio = np.random.randint(-16000, 16000, samples_24k, dtype=np.int16).tobytes()

        # Downsample to 8kHz
        pcm_8k = resample_audio(cartesia_audio, CARTESIA_SAMPLE_RATE, PLIVO_SAMPLE_RATE)
    ...
    def test_full_roundtrip(self):
        ...
        # Simulate Cartesia output at 24kHz
        pcm_24k = resample_audio(pcm_8k, PLIVO_SAMPLE_RATE, CARTESIA_SAMPLE_RATE)

        # Cartesia -> Plivo
        pcm_8k_out = resample_audio(pcm_24k, CARTESIA_SAMPLE_RATE, PLIVO_SAMPLE_RATE)
