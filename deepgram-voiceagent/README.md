...
Both servers are exposed on a public URL, so they accept only requests that come from Plivo. Both checks below always run; there is no setting to turn them off:
...
`missing X-Plivo-Signature-V3` means the request did not come from Plivo, for example a hand-made `curl`. `Rejected /ws connection: …` means the stream URL was not issued by this server's answer webhook, was altered, or is more than 5 minutes old. To call the server by hand, sign the request with your `PLIVO_AUTH_TOKEN` the way Plivo does (see `plivo_signature_headers()` in `tests/helpers.py`).
