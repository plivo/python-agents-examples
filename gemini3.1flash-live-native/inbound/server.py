...
app = FastAPI(
    title="Gemini 3.1 Flash Live Voice Agent (Inbound)",
    description="Inbound voice agent: Gemini 3.1 Flash Live S2S + Plivo + Silero VAD",
    version="0.1.0",
)
...
def configure_plivo_webhooks() -> bool:
    ...
    try:
        ...
        app_name = "Gemini31Flash_Live_Voice_Agent"
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gemini3.1flash-live-voice-agent-inbound",
...
def main() -> None:
    ...
    logger.info(f"Starting Gemini 3.1 Flash Live Inbound Agent on port {SERVER_PORT}")
