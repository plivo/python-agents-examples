"""
Voice agent using Gemini LLM, Deepgram STT, and ElevenLabs TTS.
...
ELEVENLABS_SAMPLE_RATE = 24000  # ElevenLabs outputs 24kHz PCM
        ...
        try:
            async with self._session.post(url, headers=headers, json=payload) as resp:
                if resp.status == 200:
                    ...
                    logger.debug(f"ElevenLabs TTS: synthesized {len(audio_data)} bytes")
                    return audio_data
                else:
                    error = await resp.text()
                    logger.error(f"ElevenLabs TTS error {resp.status}: {error}")
                    return b""
        except Exception as e:
...
class VoiceAgent:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            greeting = (
                "Hello! This is Alex, the Gemini-Deepgram-ElevenLabs native agent. "
                "I'm running Gemini for reasoning, "
                "Deepgram for listening, "
                "and ElevenLabs for speaking. "
                "How can I help you today?"
            )
            greeting_audio = await self._tts.synthesize(greeting)
            if greeting_audio:
                pcm_8k = resample_audio(
                    greeting_audio, ELEVENLABS_SAMPLE_RATE, PLIVO_SAMPLE_RATE
                )
                ulaw_audio = pcm_to_ulaw(pcm_8k)
                await self._audio_queue.put(ulaw_audio)
