...
import agent
from agent import DEEPGRAM_MODEL, ELEVENLABS_MODEL, ELEVENLABS_VOICE_ID, GEMINI_MODEL
...
app = FastAPI(
    title="Gemini-Deepgram-ElevenLabs Voice Agent",
    description="Voice agent using Gemini LLM, Deepgram STT, and ElevenLabs TTS with Plivo telephony",
    version="0.1.0",
)
...
def configure_plivo_webhooks() -> bool:
    ...
    try:
        ...
        app_name = "Gemini_Deepgram_ElevenLabs_Agent"
...
async def health_check() -> dict:
    """Health check endpoint with component fingerprint."""
    phone = normalize_phone_number(PLIVO_PHONE_NUMBER)
    return {
        "status": "ok",
        "service": "gemini-deepgram-elevenlabs-voice-agent",
        "phone_number": f"+{phone}" if phone else "not configured",
        "components": {
            "llm": {"provider": "google", "model": GEMINI_MODEL},
            "stt": {"provider": "deepgram", "model": DEEPGRAM_MODEL},
            "tts": {"provider": "elevenlabs", "model": ELEVENLABS_MODEL, "voice_id": ELEVENLABS_VOICE_ID},
            "telephony": "plivo",
            "framework": "native",
        },
    }
...
def main() -> None:
    """Run the server."""
    logger.info(f"Starting Gemini-Deepgram-ElevenLabs Voice Agent on port {SERVER_PORT}")
