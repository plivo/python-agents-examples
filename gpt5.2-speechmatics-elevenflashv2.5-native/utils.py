"""Audio conversion utilities and Silero VAD processor.

Plivo sends/receives mu-law 8kHz. This module converts between Plivo audio
and the formats required by Speechmatics (PCM16 8kHz), ElevenLabs (PCM16 24kHz),
and Silero VAD (float32 16kHz).
"""
...
ELEVENLABS_SAMPLE_RATE = 24000  # ElevenLabs TTS outputs 24kHz PCM
VAD_SAMPLE_RATE = 16000  # Silero VAD operates at 16kHz

# Silero VAD configuration
VAD_CHUNK_SAMPLES = int(os.getenv("VAD_CHUNK_SAMPLES", "512"))  # 32ms at 16kHz
VAD_START_THRESHOLD = float(os.getenv("VAD_START_THRESHOLD", "0.85"))
VAD_END_THRESHOLD = float(os.getenv("VAD_END_THRESHOLD", "0.35"))
VAD_MIN_SILENCE_MS = int(os.getenv("VAD_MIN_SILENCE_MS", "500"))
VAD_PRE_SPEECH_PAD_MS = int(os.getenv("VAD_PRE_SPEECH_PAD_MS", "150"))
...
def plivo_to_speechmatics(mulaw_8k: bytes) -> bytes:
    """Convert Plivo audio (μ-law 8kHz) to Speechmatics format (PCM16 8kHz).

    Speechmatics accepts 8kHz directly — no resampling needed, just μ-law decode.
    """
