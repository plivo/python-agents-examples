...
app = FastAPI(
    title="xAI Realtime-Plivo Voice Agent (Inbound)",
    description="Inbound voice agent using the xAI realtime API with Plivo telephony",
    version="0.1.0",
)
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "xai-realtime-plivo-voice-agent-inbound",
...
def main() -> None:
    ...
    logger.info(f"Starting xAI Realtime-Plivo Inbound Voice Agent on port {SERVER_PORT}")
