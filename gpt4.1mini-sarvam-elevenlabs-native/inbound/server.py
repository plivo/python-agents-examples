...
try:
    ...
    Traceloop.init(app_name="gpt4.1mini-sarvam-elevenlabs-native")
...
app = FastAPI(
    title="GPT-4.1 Mini + Sarvam + ElevenLabs Voice Agent (Inbound)",
    description="Inbound voice agent: GPT-4.1 mini + Sarvam STT + ElevenLabs TTS + Plivo",
    version="0.1.0",
)
...
def configure_plivo_webhooks() -> bool:
    ...
    try:
        ...
        app_name = "GPT4Mini_Sarvam_ElevenLabs_Voice_Agent"
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gpt4.1mini-sarvam-elevenlabs-voice-agent-inbound",
...
def main() -> None:
    ...
    logger.info(
        f"Starting GPT-4.1 Mini + Sarvam + ElevenLabs Inbound Agent on port {SERVER_PORT}"
    )
