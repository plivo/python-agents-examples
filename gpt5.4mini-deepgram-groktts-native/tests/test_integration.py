"""
Integration tests for GPT-5.4 Mini + Deepgram + Grok TTS Voice Agent.
...
3. API Integration - Test OpenAI, Deepgram, Grok TTS API connections
...
class TestUnitAudioConversion:
    ...
    def test_plivo_to_deepgram_no_resample(self):
        """plivo_to_deepgram converts u-law 8kHz to PCM16 8kHz (no resample)."""
        from utils import plivo_to_deepgram, ulaw_to_pcm

        # 160 bytes u-law = 20ms at 8kHz (one Plivo packet)
        mulaw_data = b"\xff" * 160
        result = plivo_to_deepgram(mulaw_data)
...
# =============================================================================
# UNIT TESTS - DeepgramSTT
# =============================================================================


class TestUnitDeepgramSTT:
    """Unit tests for DeepgramSTT message parsing."""
...
class TestDeepgramIntegration:
    ...
    @pytest.mark.asyncio
    async def test_deepgram_stt(self, deepgram_configured):
        """Test basic Deepgram speech-to-text with silence."""
        import aiohttp
        ...
        async with (
            ...
            session.ws_connect(url, headers=headers) as ws,
        ):
                # Send 1 second of silence (PCM16 8kHz)
                ...
                assert data.get("type") in ("Results", "Metadata")


class TestGrokTTSIntegration:
    """Integration tests for x.ai Grok TTS API."""

    @pytest.fixture
    def groktts_configured(self):
        if not XAI_API_KEY:
            pytest.skip("XAI_API_KEY not configured")

    @pytest.mark.asyncio
    async def test_groktts_unary(self, groktts_configured):
        """Test basic Grok TTS unary endpoint."""
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
