"""Inbound voice agent — GPT-5.4 mini + Deepgram STT + Grok TTS engine.

Uses a pipeline architecture:
  Plivo audio -> Deepgram STT -> GPT-5.4 mini -> Grok TTS -> Plivo audio

Grok TTS outputs u-law 8kHz directly via WebSocket streaming, so no audio
conversion is needed on the TTS side — audio goes straight to Plivo.
...
class DeepgramSTT:
    ...
    final transcript. Supports an optional on_transcript callback for
    ...
    async def send_audio(self, pcm_audio: bytes) -> None:
        """Send PCM16 8kHz audio to Deepgram."""
...
GROK_TTS_VOICE = os.getenv("GROK_TTS_VOICE", "eve")
GROK_TTS_LANGUAGE = os.getenv("GROK_TTS_LANGUAGE", "en")
...
class VoiceAgent:
    """Voice conversation session: Plivo + Deepgram STT + GPT-5.4 mini + Grok TTS."""
    ...
    async def _generate_llm_response(self, user_text: str) -> str:
        """Send conversation to GPT-5.4 mini and return text response."""
        import httpx
    ...
    @_traced("tts")
    async def _synthesize_with_groktts(self, text: str) -> None:
        """Stream text through Grok TTS WebSocket and queue audio for Plivo.

        Grok TTS is configured to output u-law 8kHz, which matches Plivo's
        native format exactly — no audio conversion needed.
        """
        import websockets
        ...
        ws_url = (
            ...
            f"?language={GROK_TTS_LANGUAGE}"
            f"&voice={GROK_TTS_VOICE}"
            f"&codec=mulaw"
            f"&sample_rate=8000"
        )
        ...
        try:
            async with websockets.connect(
                ...
                # Send text as a single delta then signal done
                ...
                async for msg in ws:
                    ...
                    if event_type == "audio.delta":
                        ...
                        # Audio is already u-law 8kHz — queue directly for Plivo
                        ...
                        self._loge("tts", f"Grok TTS error: {event.get('message', '')}")
                        break

                total_time = (time.monotonic() - t0) * 1000
                audio_duration = total_bytes / 8000  # u-law 8kHz = 1 byte per sample
                ttfb_str = ""
...
You can use the caller's phone number for SMS or callbacks without asking."""
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            if greeting:
                ...
                await self._synthesize_with_groktts(greeting)
    ...
    async def _receive_from_plivo(self) -> None:
        """Receive audio from Plivo, stream to Deepgram STT, run VAD for turn detection."""
        media_count = 0
        try:
            while self._running:
                ...
                if event == "media":
                    ...
                    if payload:
                        ...
                        # Always forward audio to Deepgram STT — even
                        ...
                        pcm_8k = plivo_to_deepgram(mulaw_audio)
        ...
        async with self._turn_lock:
            ...
            try:
                ...
                self._current_tts_task = asyncio.create_task(
                    self._synthesize_with_groktts(response_text),
                    name="tts_synthesis",
                )
