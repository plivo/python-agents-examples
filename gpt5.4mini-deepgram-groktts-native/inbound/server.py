...
app = FastAPI(
    title="GPT-5.4 Mini + Deepgram + Grok TTS Voice Agent (Inbound)",
    description="Inbound voice agent: GPT-5.4 mini + Deepgram STT + Grok TTS + Plivo",
    version="0.1.0",
)
...
def configure_plivo_webhooks() -> bool:
    ...
    try:
        ...
        app_name = "GPT54Mini_Deepgram_GrokTTS_Voice_Agent"
...
async def health_check() -> dict:
    ...
    return {
        "status": "ok",
        "service": "gpt5.4mini-deepgram-groktts-voice-agent-inbound",
...
def main() -> None:
    ...
    logger.info(
        f"Starting GPT-5.4 Mini + Deepgram + Grok TTS Inbound Agent on port {SERVER_PORT}"
    )
