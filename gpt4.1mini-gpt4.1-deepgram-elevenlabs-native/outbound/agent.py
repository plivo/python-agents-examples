...
class SmartTurnProcessor:
    ...
    def _ensure_model(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Failed to load smart-turn-v2 model: {e}. "
...
class DeepgramSTT:
    ...
    is complete — used as a convergence gate with SmartTurn so we never
    harvest a half-finished transcript.
    """
    ...
    async def wait_for_utterance(self, timeout: float = 1.0) -> bool:
        ...
        try:
            await asyncio.wait_for(
                self._utterance_complete.wait(), timeout=timeout,
            )
            return True
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    if data.get("type") == "Results":
                        ...
                        if speech_final:
                            ...
                            logger.debug(
                                "STT: speech_final — utterance complete"
                            )
...
class VoiceAgent:
    ...
    def _trim_history(self) -> None:
        ...
        self._log(
            "history",
            f"trimmed {trimmed} old messages, "
            f"{len(self._conversation_history)} remaining",
        )
...
You can use the caller's phone number for SMS or lookups without asking."""
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            logger.info(
                ...
                f"rx={self._plivo_rx_bytes} bytes, "
                f"tx={self._plivo_tx_chunks} chunks"
            )
    ...
    async def _receive_from_plivo(self) -> None:
        """Receive audio from Plivo, stream to Deepgram, run smart-turn."""
        ...
        try:
            while self._running:
                ...
                if event == "media":
                    ...
                    if payload:
                        ...
                        if media_count == 1:
                            self._log(
                                "plivo_rx", "first audio packet received"
                            )
                        ...
                        speech_started, speech_ended = (
                            self._smart_turn.process_audio(vad_audio)
                        )

                        # Barge-in: cancel any in-flight work and stop
                        # playback. Always send clearAudio — Plivo may
                        # still be playing from its internal buffer.
                        if speech_started:
                            self._last_activity = time.monotonic()
                            ...
                            # Only clear transcript when we actually
                        ...
                        if (
                            ...
                            self._log(
                                "session",
                                f"silence timeout ({idle:.1f}s) — nudging caller",
                            )
                            self._turn_count += 1
                        ...
                        if speech_ended:
                            ...
                            self._log(
                                "vad",
                                "speech END — running smart-turn analysis",
                            )
                            turn_complete = (
                                await self._smart_turn.analyze_turn()
                            )
                            if turn_complete:
                                # Wait for Deepgram to finish processing —
                                # avoids harvesting incomplete transcript.
                                dg_ready = (
                                    await self._deepgram.wait_for_utterance(
                                        timeout=1.0,
                                    )
                                )
                                if not dg_ready:
                                ...
                                transcript = (
                                    self._deepgram.latest_transcript
                                )
                                if transcript.strip():
                                    self._turn_count += 1
                                    self._turn_id = uuid.uuid4().hex[:8]
                    ...
                    self._log(
                        "plivo_rx", "received stop event — call ended"
                    )
                    break
            ...
            self._logv(
                "plivo_rx",
                f"exiting — received {media_count} media packets",
            )
    ...
    async def _send_to_plivo(self) -> None:
        ...
        try:
                ...
                try:
                    ...
                    while len(audio_buffer) >= PLIVO_CHUNK_SIZE:
                        ...
                        if self._plivo_tx_chunks == 1:
                            self._log(
                                "plivo_tx",
                                "first audio chunk sent to Plivo",
                            )
            ...
            self._logv(
                "plivo_tx",
                f"exiting — total {self._plivo_tx_chunks} chunks sent",
            )
