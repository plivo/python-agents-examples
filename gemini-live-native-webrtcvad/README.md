# Gemini Live Voice Agent with webrtcvad Turn Detection

Real-time voice agent using Google Gemini Live API for speech-to-speech conversations with Plivo telephony and **webrtcvad-based voice activity detection** for turn detection and barge-in interruption handling.

## Features

- **Speech-to-Speech**: Native audio using Gemini Live API (no separate STT/TTS)
- **Two-Layer VAD**: Client-side webrtcvad + server-side Gemini AutomaticActivityDetection
...
### Layer 1 — Client-Side VAD (webrtcvad)
- Runs on every 20ms audio frame from Plivo (before sending to Gemini)
- Detects user speech locally with zero server round-trip latency
- When speech is detected while the agent is talking, triggers instant interruption
...
1. Client-side VAD detects speech instantly (~40ms)
...
```bash
cd gemini-live-native-webrtcvad
uv sync
```
...
```
gemini-live-native-webrtcvad/
├── agent.py            # Voice agent with Gemini Live API + webrtcvad
...
4. **VAD Processing**: webrtcvad analyzes 8kHz PCM for speech detection
...
| `VAD_AGGRESSIVENESS` | webrtcvad aggressiveness (0-3) | `3` |

### VAD Tuning

- **VAD_AGGRESSIVENESS**: 0 = least aggressive (more false positives), 3 = most aggressive (fewer false positives). Default `3` works best for telephony.
...
```bash
docker build -t gemini-live-voice-agent-webrtcvad .
docker run -p 8000:8000 --env-file .env gemini-live-voice-agent-webrtcvad
```
...
- Try lowering `VAD_AGGRESSIVENESS` if speech isn't being detected
...
5. **webrtcvad frame size**: Exactly 320 bytes PCM16 at 8kHz = 20ms, which matches Plivo's μ-law frame size (160 bytes μ-law → 320 bytes PCM16).
