"""Inbound voice agent — GPT-5.4 Mini + Speechmatics STT + ElevenLabs TTS engine.
...
# =============================================================================
# Speechmatics STT — Voice Agent API via WebSocket
# =============================================================================


class SpeechmaticsSTT:
    """Real-time speech-to-text using Speechmatics Voice Agent WebSocket API.

    Streams audio continuously at 8kHz PCM and maintains the latest
    final transcript. Speechmatics sends AddSegment events for final text
    and EndOfTurn when the speaker is done. Supports an optional
    on_transcript queue for event-driven turn processing.
    """

    def __init__(self, call_id: str = "", profile: str = "adaptive"):
        ...
        self.on_end_of_turn: asyncio.Event | None = None
    ...
    async def connect(self) -> None:
        """Connect to Speechmatics Voice Agent WebSocket."""
        self._session = aiohttp.ClientSession()
        self._running = True

        url = f"wss://preview.rt.speechmatics.com/v2/agent/{self._profile}"
        headers = {"Authorization": f"Bearer {SPEECHMATICS_API_KEY}"}

        self._ws = await self._session.ws_connect(url, headers=headers)
        logger.bind(call_id=self._call_id).info(
            f"Connected to Speechmatics (profile={self._profile})"
        )
        ...
        await self._ws.send_str(json.dumps(start_msg))
        ...
        async for msg in self._ws:
            if msg.type == aiohttp.WSMsgType.TEXT:
                ...
                if data.get("message") == "RecognitionStarted":
                    logger.bind(call_id=self._call_id).info(
                        "Speechmatics RecognitionStarted"
                    )
                    break
                if data.get("message") == "Error":
                    logger.bind(call_id=self._call_id).error(
                        f"Speechmatics start error: {data}"
                    )
                    break
            elif msg.type in (aiohttp.WSMsgType.ERROR, aiohttp.WSMsgType.CLOSED):
                logger.bind(call_id=self._call_id).error(
                    "Speechmatics WebSocket closed during handshake"
                )
                break
    ...
    async def _receive_loop(self) -> None:
        """Receive transcription events from Speechmatics."""
        try:
            async for msg in self._ws:
                if msg.type == aiohttp.WSMsgType.TEXT:
                    data = json.loads(msg.data)
                    ...
                    if msg_type in ("AddPartialTranscript", "AddPartialSegment"):
                        ...
                        if not partial:
                            # Voice Agent API format fallback
                        ...
                        logger.bind(call_id=self._call_id).debug(
                            f"Speechmatics EndOfTurnPrediction: {data}"
                        )

                    elif msg_type in ("SessionMetrics", "SpeakerMetrics"):
                        logger.bind(call_id=self._call_id).debug(
                            f"Speechmatics metrics: {msg_type}"
                        )

                    elif msg_type == "AudioAdded":
                        pass  # Just an ack, ignore

                    elif msg_type == "EndOfTranscript":
                        logger.bind(call_id=self._call_id).info(
                            "Speechmatics EndOfTranscript — session closing"
                        )
                        break

                    elif msg_type == "Error":
                        logger.bind(call_id=self._call_id).error(
                            f"Speechmatics error: {data}"
                        )

                    elif msg_type == "Warning":
                        logger.bind(call_id=self._call_id).warning(
                            f"Speechmatics warning: {data}"
                        )

                    elif msg_type == "Info":
                        logger.bind(call_id=self._call_id).info(
                            f"Speechmatics info: {data}"
                        )
                        ...
                        logger.bind(call_id=self._call_id).warning(
                            f"Speechmatics unknown message: {msg_type}"
                        )

                elif msg.type == aiohttp.WSMsgType.ERROR:
                    logger.bind(call_id=self._call_id).error(
                        f"Speechmatics WebSocket error: {msg.data}"
                    )
                    break
            ...
            if self._running:
                logger.bind(call_id=self._call_id).error(
                    f"Speechmatics receive error: {e}"
                )

    async def send_audio(self, pcm_8k: bytes) -> None:
        """Send PCM16 8kHz audio as raw binary to Speechmatics."""
        if self._ws and not self._ws.closed:
            await self._ws.send_bytes(pcm_8k)
    ...
    async def close(self) -> None:
        ...
        if self._ws and not self._ws.closed:
            try:
                end_msg = {"message": "EndOfStream", "last_seq_no": 0}
                await self._ws.send_str(json.dumps(end_msg))
            except Exception:
                pass
...
class VoiceAgent:
    ...
    def __init__(
        ...
        self._stt = SpeechmaticsSTT(call_id=call_id, profile=SPEECHMATICS_PROFILE)
        self._transcript_queue: asyncio.Queue[str] = asyncio.Queue()
        self._stt.on_transcript = self._transcript_queue
        self._end_of_turn_event = asyncio.Event()
        self._stt.on_end_of_turn = self._end_of_turn_event
    ...
    async def _generate_llm_response(self, user_text: str) -> str:
        ...
        try:
            ...
            if tool_calls_map:
                ...
                # Get follow-up response after tool calls (non-streaming is fine)
                messages = [
                    {"role": "system", "content": self.system_prompt},
                    *self._conversation_history,
                ]
            ...
            self._conversation_history.append({"role": "assistant", "content": full_content})
            self._log(
                "llm",
    ...
    async def _synthesize_with_elevenlabs(self, text: str) -> None:
        ...
        try:
            async with aiohttp.ClientSession() as session, session.ws_connect(
                ...
                audio_duration = total_bytes / 8000  # mu-law 8kHz = 1 byte per sample
                ttfb_str = ""
...
You can use the caller's phone number for SMS or callbacks without asking."""
    ...
    async def _receive_from_plivo(self) -> None:
        """Receive audio from Plivo, stream to Speechmatics STT, run VAD for turn detection."""
        media_count = 0
        try:
            while self._running:
                ...
                if event == "media":
                    ...
                    if payload:
                        ...
                        # Always forward audio to Speechmatics STT — even
                        ...
                        pcm_8k = plivo_to_speechmatics(mulaw_audio)
